...
class Finger_Numbers(Dataset):
    ...
    def __init__(self, root_dir, d_type, transform=None, resize_size=(96, 96), fold_ratio=2,
        ...
        print(self.processed_folder) #add
    ...
    def __create_info_df_csv(self):
        ...
        # else:
        #     print("digitStruct.mat to CSV %s directory"%self.d_type)
        ...
        #         sys.exit()
            ...
            # digit_struct_data = Finger_Numbers.read_digit_mat(os.path.join(mat_file_path))
            ...
            self.info_df['img_width'], self.info_df['img_height'] = \
                zip(*self.info_df['img_name'].apply(lambda name: Finger_Numbers.get_image_size(
    ...
    def __gen_datasets(self):
        ...
        for _, row in self.info_df.iterrows():
            ...
            #print("square_bb_x0_selected = ",square_bb_x0_selected) #add
            square_bb_y0_selected = row['bb_y0']
            #print("square_bb_y0_selected = ", square_bb_y0_selected) #add
            square_bb_x1_selected = row['bb_x1']
            #print("square_bb_x1_selected = ", square_bb_x1_selected) #add
            square_bb_y1_selected = row['bb_y1']
            #print("square_bb_y1_selected = ", square_bb_y1_selected) #add
            ...
            if is_rectangle_wide:
                ...
                if square_bb_y1_selected > img_height:
                    print("square_bb_y1_selected(bb_y1) > img_height")  # add
                ...
                if square_bb_x1_selected > img_width:
                    print("square_bb_x1_selected(bb_x1) > img_width")  # add
            ...
            increase = round(square_bb_width * Finger_Numbers.expansion_ratio / 2.)
            ...
            try:  # add
                image = Image.open(os.path.join(self.root_dir, self.__class__.__name__, self.d_type,
                                                row['img_name']))
            except FileNotFoundError:  # add
                print("Cannot Found %s Picture" % row['img_name'])  # add
                continue  # add
...
def Fingers_get_datasets(data, load_train=True, load_test=True, resize_size=(96, 96), fold_ratio=2,
                      simplified=False):
    ...
    if load_train:
        ...
        train_dataset = Finger_Numbers(root_dir=data_dir, d_type='train',
    ...
    if load_test:
        ...
        test_dataset = Finger_Numbers(root_dir=data_dir, d_type='test',
...
def Fingers_74_get_datasets(data, load_train=True, load_test=True):
    """ Returns SVHN Dataset with 96x96 images
    """
    return Fingers_get_datasets(data, load_train, load_test, resize_size=(74, 74), fold_ratio=1)


def Fingers_74_simplified_get_datasets(data, load_train=True, load_test=True):
...
datasets = [
    ...
    {
        'name': 'Fingers_number_simplified',
        'input': (3, 74, 74),
        'output': ([1]),
        'loader': Fingers_74_simplified_get_datasets,
        'collate': Finger_Numbers.collate_fn
    }
]
